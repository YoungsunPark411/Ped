...
def CtrlMake():
    ...
    SpineFK1Ctrl = gn.ControlMaker('SpineFK1Ctrl', 'diamond', 17, exGrp=0, size=Scale*1.4)
    SpineMidCtrl = gn.ControlMaker('SpineMidCtrl', 'hexagon', 23, exGrp=0, size=Scale*1.4)
    pm.select(SpineMidCtrl[0])
    pm.addAttr(ln="SpineBlend", at='double', min=0, max=1, dv=0, k=1)
    SpineFK2Ctrl = gn.ControlMaker('SpineFK2Ctrl', 'diamond', 17, exGrp=0, size=Scale*1.4)
